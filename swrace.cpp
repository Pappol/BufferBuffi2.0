...
void placeWalls(int n, int m) {
  ...
  do {
    ...
    for (int j = border; j < m1; j++) { // BLUE  TOP
      if (matrix[border][j].type == BIANCO && matrix[border][j].U == false) {
    ...
    for (int i = border; i < n1; i++) { // BLUE  RIGHT
      if (matrix[i][m1 - 1].type == BIANCO && matrix[i][m1 - 1].R == false) {
    ...
    for (int j = border; j < m1; j++) { // BLUE  BOTTOM
      if (matrix[n1 - 1][j].type == BIANCO && matrix[n1 - 1][j].D == false) {
    ...
    for (int i = border; i < n1; i++) { // BLUE LEFT
      if (matrix[i][border].type == BIANCO && matrix[i][border].L == false) {
